...
const ConfirmReceiverTransaction: React.FC<PrintButtonProps> = ({ code, onClose, onCloseButt }) => {
  ...
  const buttonHandle = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (orderDetails && employerDetails) {
      let status: Status = {
        action: 'Điểm giao dịch đích đã nhận',
    ...
    <div>
      ...
      <div className="min-w-[350px] z-50 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white text-black p-4 rounded shadow-md">
        ...
        <div className='flex justify-center mb-4'>
          <label className='text-[15px] font-bold'>Xác nhận điểm giao dịch đích đã nhận đơn</label>
        </div>
        <div className='mb-4'>
...
export default ConfirmReceiverTransaction
    ;
